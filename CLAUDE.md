...
| 6 | **This directory is published to a PUBLIC mirror on every `stage` merge** (`mx-team-dev/tds-click-processor`; the legacy `tutoroton` mirror is still written too, and ONLY because live node 53 pinned its `origin` at clone time on 2026-07-06 — see `ANCHOR-GEOTDS-CUT-THE-CORD.md` goal A9), which freshly-provisioned nodes `git clone`. A gitleaks scan of the subtree gates the push. Test deps are kept out of `requirements.txt` on purpose so the edge image stays lean | `.github/workflows/click-processor-mirror.yml:1-13`, `:41-50`; `requirements-dev.txt:1-9` |
...
| 8 | 🔴 **The returning-user layer is LIVE — this row said the opposite until 2026-09-04.** `returning_resolver_enabled` **defaults to `True`**, and both node deploy scripts force it on rather than leaving it to the default. So `identity.py`, `sticky.py` and `history.py` DO run, and a session that read this row would have reasoned about production from a dark-by-default premise that has not held for some time. Measured live the same day: a returning visitor was served the returning funnel 10/10, matching its control exactly. The kill-switch is real and still the thing to check — it is simply ON. ⚠️ Note this row already contradicted **row 9** below, which describes identity/sticky writes being driven in a test; a table that disagrees with itself is how a stale claim survives | `config.py:641` (`= True`), `deploy/update.sh:102`, `deploy/render-env.sh:98`; live repro 2026-09-04 |
| 9 | **A test that proves "this path writes NOTHING" must wrap EVERY pool in hazard 2, and only a MUTATION proves it does.** Measured 2026-08-31: a write-detector wrapping only `router.get_redis` reported zero writes while a deliberately planted `visitor_id` drove `identity.py`/`sticky.py` writes through `get_identity_redis()` — 15/15 green, detector blind. Two probes are not independent if they watch one pool. The knowledge was already in row 2 of this very table; a purity claim still needs the mutation | `tests/unit/test_route_code_honoured.py` (`FakeRoutingRedis` + `FakeIdentRedis`, plus `TestTheDetectorsThemselvesGoRed`); `redis_client.py:11`, `:54`, `:102` |
| 10 | **`TestClient(app)` used as a CONTEXT MANAGER runs lifespan** and dials a real Redis; construct it bare, as the rest of this service's tests do. And **fakeredis binds a CLIENT to an event loop while the DATA lives on the server** — reuse one `FakeServer` and hand each loop a fresh client, or the second `asyncio.run` raises "bound to a different event loop" | `tests/unit/test_route_preview_endpoint.py` |
| 11 | 🔴 **A LIVE probe over plain `http` gives every request a NEW visitor, so every stickiness or returning claim made from it is VACUOUS — and it presents as a pass.** The node sets `_tds_vid` / `_tds_cid` / `_tds_id` with **`Secure`**, so a `curl -b/-c` jar on `http://` stores nothing and each request is a first-time visitor. Measured 2026-09-07: an `http` probe produced a different `uid` on every row and `sticky_status='na'` throughout, while its own control — *"two plain clicks returned the same target, so the pin holds"* — read GREEN, because a campaign with one default flow and one target returns the same target BY CONSTRUCTION. That control could not have failed. The same probe over `https` stabilised the `uid` immediately and flipped `is_returning` 0→1. ⚠️ **Two things wear the word *pin*:** `sticky_status` (the sticky pin) and `target_selection_path='pinned'` (the deterministic selection path) — reading the second as evidence of the first is what made a false claim look measured. **Verify a stickiness claim from `tds.events` (`sticky_status` `minted`/`hit`, `target_selection_path`), never from URL equality**, and remember `sticky_active` additionally requires the campaign's `returning_mode == 'sticky'` — `fresh` yields `sticky_status='na'` even with a stable identity | `router.py:2069-2075`; live account + both corrected arms in `docs/development/offerwall-2026-09-04/50-E1-CORRECTED-THE-PIN-IS-REAL.md` |
| 12 | ⚠️ **The edge answers a default Python User-Agent with `403`, so a probe written in `urllib` can silently truncate or empty its own result — and the emptiness reads as a fact about the world.** Measured 2026-09-08 on one URL in the same second: `curl` default UA → **302**, `-A "Python-urllib/3.12"` → **403**. Found by `b11a2c99` when a paging loop rewritten in `urllib` would have dropped pages mid-set; reproduced independently here. 🔴 **The mechanism is NOT in our code** — `grep` finds no User-Agent or 403 handling in `services/worker/src/index.js` or `click-processor/app/`, and no ADR mentions it; most plausibly Cloudflare bot management upstream of the Worker, **UNVERIFIED — do not repeat that as the cause**. What IS measured is the behaviour. Probe the edge with `curl`, or set an explicit UA; and give any paging probe a completeness check (`len(items) == total`) so a truncated read announces itself instead of presenting as a smaller world | reproduce: `curl -sk -o /dev/null -w '%{http_code}' https://<campaign-domain>/<path>` with and without `-A "Python-urllib/3.12"` |
